...
req_modules=["Tkinter",'datetime','time','tkMessageBox','pyserial','thread','ttk','platform',"PIL",'Image',"sys","os","opencv-python"]
...
#from operator import itemgetter3
#===============  plot  ================#
import numpy as np
...
var_output= IntVar()
output_chk= Checkbutton(fn_btns_frame,bd=2 ,text="Optical Testing", variable = var_output,\
                                 onvalue =1, offvalue = 0, \
                                 bg='grey52',fg='#300302',padx=5)
output_chk.pack(side="left",ipadx=2)
output_chk_ttp = CreateToolTip(output_chk, " Real Time optical testing  ")
...
def hi_lo(frame):
    ...
    for i in range(0,(refPt[1][0]-refPt[0][0]),(refPt[1][0]-refPt[0][0])//20):
    ...
    for j in range(20):
        ...
        #print ten[j]
        khali=[]
        height,width,hgjs = ten[j].shape

        for i in range(4):
            max = 0
            for k in range(5):
                for f in range(5):
                    if(max<ten[j][(height/4)*i+(height/26)*(f+1),width*(k+1)/6,0]+ten[j][(height/4)*i+(height/26)*(f+1),width*(k+1)/6,1]+ten[j][(height/4)*i+(height/26)*(f+1),width*(k+1)/6,2]):
                        max =ten[j][(height/4)*i+(height/26)*(f+1),width*(k+1)/6,0]+ten[j][(height/4)*i+(height/26)*(f+1),width*(k+1)/6,1]+ten[j][(height/4)*i+(height/26)*(f+1),width*(k+1)/6,2]
            khali.append(max)
        maxx[j] = khali
...
def change_arr(arr):
  ...
  for j in range(3):
    for i in range(10):
      check_arr.append(int(arr[i][j+0]))
      check_arr.append(int(arr[i][j+3]))
  for i in range(10):
    check_arr.append(int(arr[i][6]))
    check_arr.append(int(arr[i][7]))
...
#    print (btn_color),'len btn_co'
      ...
      check_arr = change_arr(arr)
      for i in range(0,total_cell*no_pin)  :
        if  100<fb[i]<200:
            if check_arr[i]==pin_arr[i]:
                btn_color.append('#006633')#green
            else:
                btn_color.append('#002ccc')#blue
        elif fb[i]==0:
          if check_arr[i]==pin_arr[i]:
            btn_color.append('#006633')#green
          else:
            btn_color.append('#cc5f00')#orange
        else:
          if check_arr[i]==pin_arr[i]:
            btn_color.append('#ccc100')#yellow
          else:
            btn_color.append('#cc0000')#red
...
def Acctuate():
    ...
    try:
        ...
        tkMessageBox.showinfo("WARNING", "Enter Module ID first. ")
        return
    if var_output.get()==True:
        if fil=="Connected":
            if(not tkMessageBox.askokcancel("Confirm", "Have you caliberated the camera?")):
                return
        else:
            tkMessageBox.showinfo("WARNING","Camera is not connected")
            return
...
def click_and_crop(event, x, y, flags, param):
	...
	global refPt, cropping,imageimage
	...
	# check to see if the left mouse button was released
		...
		cv2.rectangle(imageimage, refPt[0], refPt[1], (0, 255, 0), 2)
		cv2.imshow("Camera View, Press 'r' to refresh & 'c' to exit.", imageimage)
        #cv2.waitKey(0)
...
#         lblHCC.config(font=('arial',8,'bold'),bg='grey99',fg='green4',text='Successfu..',width=14,anchor='center')
#    else:
...
def HC_check():
    ...
    '''cmd1 = "python cam_check.py"
                  ...
                  if pr == "Connected":
                    lblcamera.config(font=('arial',9,'bold'),bg='grey99',fg='green4',text='Connected',width=14,anchor='center')
                    #print('Seial_Port_was_already_closed')
                    #ser.close()
                  else:
                    lblcamera.config(font=('arial',9,'bold'),bg='grey99',fg='red2',text='Disconnected',width=14,anchor='center')
                    #tkMessageBox.showwarning("WARNING", "   Port "+Port.get()+"  not Available!  or Busy!  '''

    global fil
    file = open(os.path.join(DIR,'camera.txt'),'r')
    fil = file.readline()
    if(fil=='Disconnected'):
      lblcamera.config(font=('arial',9,'bold'),bg='grey99',fg='red2',text='Disconnected',width=14,anchor='center')
    else:
      lblcamera.config(font=('arial',9,'bold'),bg='grey99',fg='green4',text=fil,width=14,anchor='center')
    file.close()
    global Port,root
...
#    print HCC_flag,'HCC_flag'
    ...
    if HCC_flag==True:
#         lblHCC.config(bg='green4',fg='grey99',text='HOST COMMUNICATION CHECK',width=28)
         lblHCC.config(font=('arial',9,'bold'),bg='grey99',fg='green4',text='Connected',width=14,anchor='center')
    else:
#         lblHCC.config(bg='red3',fg='grey99',text='HOST COMMUNICATION CHECK',width=28)
         lblHCC.config(font=('arial',9,'bold'),bg='grey99',fg='red2',text='Disconnected',width=14,anchor='center')
#    time.sleep(1)
    root.after(1000,HC_check)
#global lblcamera
import time
...
btnHCC=ttk.Button(f3 ,text=" MODULE " ,command=HC_check )
...
#======================Process Camera========================#
btncamera=t.Label(f3,font=('arial',9),text=" CAMERA ",bd=2,bg='grey90',
                     highlightthickness=2,anchor="nw",relief=t.RIDGE)
#btncamera=ttk.Button(f3 ,text=" CAMERA " ,command=camera_check )
btncamera.pack(side="left")

lblcamera= t.Label(f3,font=('arial',7,'bold'),text="                ",bd=3,bg='light sky blue' ,width=14,fg='black',anchor="nw",pady=4,relief=t.SUNKEN,justify='center')
lblcamera.pack(side="left")
#===============================================================#
btnCaliberate=ttk.Button(f3 ,text=" CALIBERATE CAMERA" ,command=caliberate )
btnCaliberate.pack(side="left")
...
btncolor1=t.Label(f3,font=('arial',9),text="  ",bd=2,bg='#006633',
                   highlightthickness=2,anchor="nw",relief=t.RIDGE)
btncolor1.place(x=1150,y=100)
btncolor11=t.Label(f3,font=('arial',9),text="checkpass",bd=2,bg='grey90',
                   highlightthickness=2,anchor="nw",relief=t.RIDGE)
btncolor11.place(x=1200,y=100)
btncolor2=t.Label(f3,font=('arial',9),text="  ",bd=2,bg='grey90',
                   highlightthickness=2,anchor="nw",relief=t.RIDGE)
btncolor2.place(x=1150,y=130)
btncolor21=t.Label(f3,font=('arial',9),text="checkbutfail",bd=2,bg='grey90',
                   highlightthickness=2,anchor="nw",relief=t.RIDGE)
btncolor21.place(x=1200,y=130)
btncolor3=t.Label(f3,font=('arial',9),text="  ",bd=2,bg='grey90',
                   highlightthickness=2,anchor="nw",relief=t.RIDGE)
btncolor3.place(x=1150,y=160)
btncolor4=t.Label(f3,font=('arial',9),text="  ",bd=2,bg='grey90',
                   highlightthickness=2,anchor="nw",relief=t.RIDGE)
btncolor4.place(x=1150,y=190)
btncolor5=t.Label(f3,font=('arial',9),text="  ",bd=2,bg='grey90',
                   highlightthickness=2,anchor="nw",relief=t.RIDGE)
btncolor5.place(x=1150,y=220)
#root.update()
