...
var_output= IntVar()
output_chk= Checkbutton(fn_btns_frame,bd=2 ,text="Opltical Testing", variable = var_output,\
...
#=========================================================================================
def hi_lo(frame,FRAME_HEIGHT,FRAME_WIDTH):
    ten = []
    #Slice current frame(photo) in 10 equal parts (vertically)
    for i in range(0,FRAME_WIDTH,FRAME_WIDTH//20):
        #tenvs.append(frame[i:i+(FRAME_WIDTH//10),FRAME_HEIGHT-FRAME_WIDTH/5:,:]) #finally (pins at top)
        ten.append(frame[FRAME_WIDTH*100/155:,i:(i+(FRAME_WIDTH/20))]) #only for now (pins at bottom)
    #---------------------------------------------------------------------------
    #print len(ten)
    ...
    for j in range(20):
        ...
        for i in range(4):
            ...
            for k in range(3):
                for f in range(5):
                    if(max<ten[j][(height/4)*i+(height/20)*f,width*(k+1)/4,0]+ten[j][(height/4)*i+(height/20)*f,width*(k+1)/4,1]+ten[j][(height/4)*i+(height/20)*f,width*(k+1)/4,2]):
                        max =ten[j][(height/4)*i+(height/20)*f,width*(k+1)/4,0]+ten[j][(height/4)*i+(height/20)*f,width*(k+1)/4,1]+ten[j][(height/4)*i+(height/20)*f,width*(k+1)/4,2]
            khali.append(max)
        maxx[j] = khali
...
#        print fb,'  :fb in Acctuate',' ,len ',len(fb)
        ...
        if(var_output.get()==True):
            ...
            FRAME_HEIGHT,FRAME_WIDTH,lhsh = frame.shape
            arr = hi_lo(frame,FRAME_HEIGHT,FRAME_WIDTH)
